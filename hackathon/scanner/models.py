...
class RTO(models.Model):
    code = models.PositiveIntegerField(max_length = 2, min_length = 2, )
    def __str__(self):
        return f"{self.code}";
...
class State(models.Model):
    ...
    RTO_code = models.ManyToManyField(RTO, on_delete = models.CASCADE, blank = False, default = None, )
    def __str__(self):
        return f"{self.state}";
...
class License(models.Model):
    DL_No = models.PositiveIntegerField(max_length = 7, min_length = 7, )
    ...
    category = models.ManyToMany(Category, blank = True, )
